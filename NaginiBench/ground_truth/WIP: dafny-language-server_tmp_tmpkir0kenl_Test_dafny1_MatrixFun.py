...
def MirrorImage(m : List[List[int]]) -> None:
    ...
    Requires(Forall(int, lambda m0: Implies(m0 >= 0 and m0 < len(m), Acc(list_pred(m[m0])))))
    Requires(len(m) > 0)
    Requires(Forall(int, lambda d_0_i_: Implies(d_0_i_ >= 0 and d_0_i_ < len(m), Acc(list_pred(m[d_0_i_])) and len(m[0]) == len(m[d_0_i_]))))
    Ensures(Acc(list_pred(m)))
    Ensures(Forall(int, lambda m0: Implies(m0 >= 0 and m0 < len(m), Acc(list_pred(m[m0])))))
    Ensures(len(m) > 0)
    Ensures(Acc(list_pred(m[0])))
    Ensures(Acc(list_pred(Old(m))))
    Ensures(len(m) == Old(len(m)))
    Ensures(Forall(int, lambda m0: Implies(m0 >= 0 and m0 < len(m), Acc(list_pred(Old(m[m0]))))))
    Ensures(Forall(int, lambda d_0_i_:
        Forall(int, lambda d_1_j_:
            Implies((((0) <= (d_0_i_)) and ((d_0_i_) < (len((m))))) and (((0) <= (d_1_j_)) and ((d_1_j_) < (len((m)[0])))), 
                    Acc(list_pred(m[d_0_i_])) and 
                    (len((m)[0])) == len(m[d_0_i_]) and 
                    ((m)[d_0_i_][d_1_j_]) == (Old((m)[d_0_i_][((len((m)[0])) - (1)) - (d_1_j_)]))))))
    d_2_a_ = int(0) # type : int
    d_2_a_ = 0
    while (d_2_a_) < (len((m))):
        Invariant(Acc(list_pred(m)))
        Invariant(Forall(int, lambda m0: Implies(m0 >= 0 and m0 < len(m), Acc(list_pred(m[m0])))))
        Invariant(len(m) > 0)
        Invariant(len(m) == Old(len(m)))
        Invariant(Forall(int, lambda d_0_i_: Implies(d_0_i_ >= 0 and d_0_i_ < len(m), Acc(list_pred(m)) and Acc(list_pred(m[d_0_i_])) and len(m[d_0_i_]) == Old(len(m[d_0_i_])))))
        Invariant(Forall(int, lambda d_0_i_: Implies(d_0_i_ >= 0 and d_0_i_ < len(m), Acc(list_pred(m)) and Acc(list_pred(m[d_0_i_])) and len(m[0]) == len(m[d_0_i_]))))
        Invariant(Acc(list_pred(m[0])))
        Invariant(Forall(int, lambda d_0_i_: Implies(d_0_i_ >= 0 and d_0_i_ < len(m), 
                                                    d_0_i_ >= 0 and d_0_i_ < len(m) and 
                                                     Acc(list_pred(m[d_0_i_])) and 
                                                     Acc(list_pred(m[0])) and
                                                    (len((m)[0])) == len(m[d_0_i_]))))
        ...
        Invariant(Forall(int, lambda d_3_i_:
            Forall(int, lambda d_4_j_:
                Implies(((((0) <= (d_3_i_)) and ((d_3_i_) < (d_2_a_))) and ((0) <= (d_4_j_))) and ((d_4_j_) < (len((m)[0]))), 
                        ((((0) <= (d_3_i_)) and ((d_3_i_) < Old(len(m)))) and ((0) <= (d_4_j_))) and ((d_4_j_) < Old(len((m)[0]))) and 
...
#         Invariant(Acc(list_pred(m)))
#         Invariant(Implies(((d_17_a_) < (d_18_b_)) and ((d_18_b_) <= (d_16_N_)), 
