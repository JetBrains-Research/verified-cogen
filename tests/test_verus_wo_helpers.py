    ...
    // post-conditions-end
{
    ...
    assert verus_lang.generate_validators(code, False) == dedent(
        """\
        verus!{
        spec fn intersperse_spec_copy_pure(numbers: Seq<u64>, delimiter: u64) -> (result:Seq<u64>)
            decreases numbers.len(),
        {
            if numbers.len() <= 1 {
                numbers
        ...
        spec fn intersperse_quantified_copy_pure(numbers: Seq<u64>, delimiter: u64, interspersed: Seq<u64>) -> (result:bool) {
            (if numbers.len() == 0 {
                ...
                interspersed.len() == 2 * numbers.len() - 1
            }) && (forall|i: int| 0 <= i < numbers.len() ==> #[trigger] interspersed[even_copy_pure(i)] == numbers[i])
                && (forall|i: int|
                0 <= i < numbers.len() - 1 ==> #[trigger] interspersed[odd_copy_pure(i)] == delimiter)
        }

        fn intersperse_valid(numbers: Vec<u64>, delimiter: u64) -> (result: Vec<u64>)
            // post-conditions-start
            ensures
                result@ == intersperse_spec_copy_pure(numbers@, delimiter),
            // post-conditions-end
